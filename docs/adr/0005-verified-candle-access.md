...
Accepted (Milestone 0.5); corrected by Milestone 0.5A
...
3. **Only manifest-v2 canonical Binance imports are queryable.** Supported datasets require `manifest_version=2`, provider `binance`, market/exchange `binance_futures`, data type `contract_klines`, endpoint `/fapi/v1/klines`, canonical schema version `1`, and `text/csv` partitions. Raw source-page artifacts must retain `application/json` media-type metadata.
...
5. **Canonical reserialization is required.** Semantically valid but non-canonical CSV (CRLF, BOM, trailing zeroes, signed zeros such as `-0`, alternate timestamp forms, missing final newline, etc.) is rejected so one logical sequence has one byte representation. Every finite decimal numerically equal to zero serializes as `"0"`.
...
7. **Pagination uses exclusive open-time cursors.** Responses include at most `limit` candles. Matching collection retains at most `limit + 1` candle references and stops immediately after the lookahead match; `next_cursor` is the last returned open time. Offset pagination is avoided because it is unstable for immutable sequences and invites skip/duplicate bugs.
8. **Query models enforce their own invariants.** `CandleQuery.__post_init__` validates UUID/symbol/timeframe/limit/UTC alignment/range/cursor rules. The service boundary re-validates so callers cannot bypass the builder.
9. **Parquet and caching are deferred.** Full-artifact read and verification before serving a page is acceptable within the existing import-size guardrail. Unbounded process-global caches and Parquet/indexed access are out of scope.
10. **Read operations are not audited.** Ordinary candle queries and `verify-snapshot` do not append audit events and do not mutate datasets. Publication remains the audited boundary.
...
- Pagination memory is bounded by the page size rather than the full filtered range.
