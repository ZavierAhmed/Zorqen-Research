...
Accepted (Milestone 0.6); hardened by Milestone 0.6A
...
12. **Sanitized provider boundary (0.6A).** Provider return values are coerced and validated. `None`, strings/bytes, non-iterables, unknown objects, multiple intents, and invalid intent fields raise `BacktestValidationError`. Unexpected provider exceptions are wrapped as `BacktestExecutionError` with the original cause preserved. Intents are intrinsically validated at construction and revalidated by the engine before use.
13. **Strictly positive normalized prices (0.6A).** After every tick/slippage normalization, entry fill, exit fill, stop-loss, and take-profit prices must be finite and strictly greater than zero. Invalid values fail closed before a fill or trade is appended; prices are never silently clamped to one tick. Policy requires `0 <= market_slippage_bps < 10000` so sell-side slippage cannot wipe a reference price by itself, and post-normalization checks still catch large tick floors to zero.
