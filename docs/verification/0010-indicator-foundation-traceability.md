...
Milestone: **1.0 / 1.0A — Deterministic Indicator Series Foundation**  
Base commit (1.0): `9b5f030c1209f435b481f60ed8571128f11a50be`  
Corrective base (1.0A): `b844c76595af333b44697a23eb93708735928661`
...
| Input is an exact tuple | `type(candles) is tuple` | `test_exact_tuple_and_candle_types_required`, `test_indicator_input_rejects_mutable_list` | PROVEN BY AUTOMATED TEST |
| Input contains exact Candle objects | `type(candle) is Candle` | `test_exact_tuple_and_candle_types_required`, `test_candle_subclass_rejected`, `test_indicator_input_rejects_non_candle_item` | PROVEN BY AUTOMATED TEST |
| Stored input cannot change after hashing | retain exact verified tuple identity | `test_stored_input_preserves_exact_tuple_identity` | PROVEN BY AUTOMATED TEST |
...
| Indicator code cannot be paired with arbitrary values | no public `from_calculation`; calculator-owned `_calculated_indicator_series` | `test_public_api_has_no_from_calculation_factory`, `test_public_api_cannot_supply_arbitrary_ema_values`, `test_series_rejects_direct_forged_construction` | PROVEN BY AUTOMATED TEST |
| Parameters are code-specific | `_canonicalize_parameters` | `test_true_range_rejects_unrelated_period_parameter`, `test_ema_rejects_missing_period_and_false_warmup`, `test_mixed_parameter_key_types_and_unsafe_text_rejected` | PROVEN BY AUTOMATED TEST |
| Warmup shape is code-specific | `_enforce_code_invariants` | `test_ema_rejects_missing_period_and_false_warmup`, `test_public_api_cannot_supply_arbitrary_ema_values` | PROVEN BY AUTOMATED TEST |
| TR/ATR non-negativity is intrinsic | `_enforce_code_invariants` | `test_negative_atr_values_rejected`, `test_calculators_still_produce_nonnegative_tr_atr`, `test_true_range_first_candle_and_gaps` | PROVEN BY AUTOMATED TEST |
| Every accepted result serializes to UTF-8 | `serialize_indicator_series` | `test_accepted_result_always_serializes_to_utf8`, `test_nonlinear_ema_differs_from_rolling_sma` | PROVEN BY AUTOMATED TEST |
| Values length / float / int / bool / NaN / Inf / Decimal subclass rejected | `_validate_values` | `test_series_rejects_values_length_mismatch`, `test_series_rejects_float_int_bool_values`, `test_series_rejects_nan_and_infinity`, `test_decimal_subclass_rejected_as_result_value`, `test_redteam_float_and_non_finite_smuggled_into_result`, `test_redteam_result_value_length_mismatch` | PROVEN BY AUTOMATED TEST |
...
| Nonlinear EMA proves recursive calculation | `ema-recursive` golden | `test_nonlinear_ema_differs_from_rolling_sma`, CLI `ema-recursive` | PROVEN BY AUTOMATED TEST |
...
| Literal goldens + CLI | `goldens.py` + `zorqen-indicators` | `test_all_indicator_golden_scenarios_pass`, `test_indicator_cli_all_emits_json_and_exits_zero`, `test_indicator_cli_unknown_scenario_nonzero` | PROVEN BY AUTOMATED TEST |
| Value mismatch fails | golden checker + CLI | `test_golden_value_mismatch_raises_and_cli_exits_nonzero`, `test_golden_atr_value_and_hash_mismatch`, `test_paired_extrema_value_and_hash_mismatch` | PROVEN BY AUTOMATED TEST |
| Hash mismatch fails | golden checker + CLI | `test_golden_atr_value_and_hash_mismatch`, `test_paired_extrema_value_and_hash_mismatch`, `test_scenario_all_mismatch_exits_nonzero_on_stderr` | PROVEN BY AUTOMATED TEST |
| `--scenario all` mismatch exits nonzero | CLI | `test_scenario_all_mismatch_exits_nonzero_on_stderr`, `test_redteam_cli_unknown_scenario_and_mismatch` | PROVEN BY AUTOMATED TEST |
...
| 20 | CLI mismatch / unknown scenario | Nonzero exit + JSON error on stderr | Passed | `test_redteam_cli_unknown_scenario_and_mismatch` | CLI checks |
| 21 | Tuple subclass / mutating iter / lying len | Rejected before access | Failed closed | `test_exact_tuple_and_candle_types_required` | Exact `type is tuple` |
| 22 | Candle subclass | Rejected | Failed closed | `test_candle_subclass_rejected` | Exact `type is Candle` |
| 23 | Public arbitrary EMA values | No public values factory | Failed closed | `test_public_api_cannot_supply_arbitrary_ema_values` | Calculator-owned assembly |
| 24 | Negative ATR via builder | Rejected | Failed closed | `test_negative_atr_values_rejected` | Non-negativity invariant |
| 25 | Golden value/hash mismatch | Nonzero + stderr JSON | Failed closed | `test_golden_value_mismatch_raises_and_cli_exits_nonzero`, `test_scenario_all_mismatch_exits_nonzero_on_stderr` | Golden checker |
