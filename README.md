...
Milestone **1.3** resolves the authoritative Adaptive MTF baseline contract from pinned MOMO Quant commit `766e31db73bbb130d12ba84f1568745210db6155`:

- Machine-readable `baselines/adaptive_mtf_trend_breakout/v1/` contract + source evidence
- Parity fixtures under `tests/fixtures/adaptive_mtf_trend_breakout/v1/`
- Approved `StrategyDefinition` bound to the contract hash
- `zorqen-strategy verify-baseline --family adaptive_mtf_trend_breakout`
- **No strategy provider** — Adaptive MTF signal/state-machine implementation remains Milestone 1.4

Milestones **1.2 / 1.2A** remain the provenance-sealed MTF indicator composition layer.

**Still not implemented:** Adaptive MTF / Support-Resistance strategy providers, dynamic provider factories, limit/maker orders, funding/leverage, backtest persistence/API/UI, campaigns, candidates, scoring, worker job leasing, autonomous research, or MOMO Quant integration.
...
## Authoritative Adaptive MTF baseline (Milestone 1.3)

Pinned MOMO Quant authority: commit `766e31db73bbb130d12ba84f1568745210db6155`. Resolution status: **RESOLVED**.

Artifacts:

- `baselines/adaptive_mtf_trend_breakout/v1/baseline_contract.json`
- `baselines/adaptive_mtf_trend_breakout/v1/source_evidence.json`
- `baselines/adaptive_mtf_trend_breakout/v1/approved_definition.json`
- `tests/fixtures/adaptive_mtf_trend_breakout/v1/` (parity CSVs + expected traces)

```bash
uv run zorqen-strategy verify-baseline --family adaptive_mtf_trend_breakout
```

Parity candle series are generated by `tools/generate_adaptive_mtf_baseline_artifacts.py`, a deterministic port of MOMO `AdaptiveDefaultFixtures`. No Adaptive MTF strategy provider is registered.
...
Executable Adaptive MTF baseline artifacts are checked in under `baselines/adaptive_mtf_trend_breakout/v1/` (`RESOLVED` against pinned MOMO Quant). Support/Resistance remains unresolved. No strategy provider is registered yet.
