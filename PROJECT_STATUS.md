...
- Default branch: `main`
- Current branch: `main`
- Previous verified commit (Milestone 1.0): `b844c76595af333b44697a23eb93708735928661`
- Milestone 1.0A base commit: `b844c76595af333b44697a23eb93708735928661`
...
Family metadata remains seeded. Milestone 1.0A seals indicator provenance only — no executable Adaptive MTF / Support-Resistance algorithms or provider-safe indicator views.

## 4. Current Milestone

- Milestone: `1.0A` — Seal indicator provenance and golden verification
- Status: Complete pending independent review and all four GitHub checks green after push
- Base: `b844c76595af333b44697a23eb93708735928661`
- Work completed:
  - Exact `tuple` / `Candle` runtime types for `IndicatorInput`
  - Removed public `IndicatorSeries.from_calculation`; calculator-owned `_calculated_indicator_series`
  - Code-specific parameter, warmup, and TR/ATR non-negativity invariants
  - Canonical parameter/UTF-8 boundary hardening
  - Nonlinear `ema-recursive` golden + mismatch CLI/stderr proofs
  - Traceability 0010 updated (0 NOT TESTED)
- Implementation started: Yes
- Repository commit created: Yes (this milestone commit)

## 5. Latest Verified State

Fixture / dataset / strategy-definition / resampling / alignment / MTF hashes: unchanged from Milestone 1.0.
...
Indicator golden result hashes:

```text
ema_close:             982dcb739655d2eb018e74911c8d53a66a9f86555ffa74aa8111c7134482d303
ema_recursive (new):   c59617e531bf61918fbfe349774618df18259f76723275f3d543ac3241779f96
...
Awaiting independent review of Milestone 1.0A and confirmation that all four GitHub checks are green after push.

## 10. Verification Evidence (Milestone 1.0A)
...
uv run pytest tests/integration/test_artifact_filesystem.py -q
uv run pytest tests/integration -q -m integration
uv run pytest tests/integration/test_candle_query.py -q
...
uv run zorqen-backtest run-mtf-golden --scenario all           # twice
uv run python -m zorqen_research.worker --check
frontend npm ci / lint / test --run / build
docker compose nginx live/ready/families
Linux Quality sequence (node:22-bookworm)
```
...
None until Milestone 1.0A is independently accepted. Milestone 1.1 is not authorized.
