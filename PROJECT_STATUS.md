...
- Default branch: `main`
- Current branch: `main`
- Previous verified commit (Milestone 0.6A): `a87a7307792d84554ce40c4f72f5e61a64d4d7a8`
- Milestone 0.6B base commit: `a87a7307792d84554ce40c4f72f5e61a64d4d7a8`
...
- Milestone: `0.6B` — Bound and Sanitize the Decision-Provider Contract
- Status: Complete pending independent review
- Base: `a87a7307792d84554ce40c4f72f5e61a64d4d7a8`
- Defects corrected (from 0.6A partial acceptance):
  - Provider-raised `BacktestValidationError` / `BacktestExecutionError` could bypass wrapping
  - Provider output was coerced via full `tuple(iterator)` materialization (hang risk on infinite generators; lazy iteration exceptions could escape)
- Work completed:
  - Two-stage boundary: wrap every `on_bar_close()` exception as sanitized `BacktestExecutionError("Decision provider failed")`
  - Exact-tuple output contract: non-tuples rejected without iteration; `len > 1` rejected; no materialization of arbitrary iterables
  - Provider-boundary tests expanded (exception types, containers, generators)
  - ADR 0006 updated for exact-tuple contract and execution wrapping
- Implementation started: Yes
- Repository commit created: Yes (this milestone commit)

## 5. Latest Verified State

- Candle querying and dataset hashes unchanged
- Golden CLI `run-golden --scenario all` exits 0 with byte-identical repeated output
- All seven golden economics and hashes unchanged from 0.6 / 0.6A
...
Awaiting independent review of Milestone 0.6B.
No later milestone is authorized.

## 10. Verification Evidence (Milestone 0.6B)
...
uv run mypy src
uv run pytest tests/unit -q                                          # 303 passed
...
uv run pytest tests/integration/test_candle_query.py -q              # 5 passed (hashes unchanged)
...
None until Milestone 0.6B is independently accepted. Milestone 0.7 is not authorized.
